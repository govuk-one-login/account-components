...
import { Algorithms, SignatureTypes, Kids, JWKS_TIMEOUT_MILLISECONDS, HttpCodesEnum } from '../types/common.js';
        ...
        for (const keyType of JWKS_KEY_TYPES) {
            ...
            if (!publicKeyMap.has(kid)) {
                try {
                    ...
                    logger.error(`Failed to retrieve ${kty} public key from SSM`, { error } as any);
            ...
            try {
                ...
                logger.error(`Failed to export the ${kty} jwk`, { error } as any);
